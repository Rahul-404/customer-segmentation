from customer_segmentation.ml.model.s3_estimator import CustomerClusterEstimator
from customer_segmentation.logger import logging
from customer_segmentation.entity.config_entity import DataTransformationConfig , ModelTrainerConfig
from customer_segmentation.constant.training_pipeline import *
from customer_segmentation.entity.config_entity import training_pipeline_config
from customer_segmentation.entity.config_entity import Prediction_config, PredictionPipelineConfig
...
class CustomerData:
    ...
    def get_input_dataset(self, column_schema: List, input_data):
        columns = column_schema#.keys()
        # created a blank dataframe with the column names from the schema
        input_dataset = pd.DataFrame([input_data], columns = columns)
        for key, value in zip(column_schema, input_data):
...
class PredictionPipeline:
    ...
    def prepare_input_data(self, input_data:list) -> pd.DataFrame:
        ...
        """
        try:
            # con verting list of data to dataframe
